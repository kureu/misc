...
local flags = {
    ...
    ["Weapon"] = false;
    ["Skeletons"] = false;["Skeletons_Color"] = { Color = rgb(16, 0, 247) };["Distance_Color"] = { Color = rgb(0, 255, 0) };["Weapon_Color"] = { Color = rgb(0, 255, 255) };
    ["InvisibleCheck"] = false;
    ["WallCheck"] = false;  
    ["TeamCheck"] = false;
    ["AliveCheck"] = false;
    ["ForceFieldCheck"] = false;["InvincibleCheck"] = false;
}
    ...
    -- Functions 
        ...
        function esp.refresh_elements( )
            for _,v in players:GetPlayers() do 
                ...
                local _hum = path and path.info and path.info.humanoid
                if _hum and _hum.Health then
                    local _mul = _hum.Health / math.max(_hum.MaxHealth, 1)
                    objects["healthbar"].BackgroundColor3 =
                        flags["Health_Low"].Color:Lerp(flags["Health_High"].Color, _mul)
                end
        ...
        esp.connection = run.RenderStepped:Connect(function()
            ...
            for _, player in players:GetPlayers() do 
                ...
                -- ==== 1. TEAM, INVISIBLE, ALIVE, FORCEFIELD, & INVINCIBLE CHECK ====
                local is_teammate = flags["TeamCheck"] and player.Team ~= nil and player.Team == players.LocalPlayer.Team
                local is_invisible = false
                local is_dead = flags["AliveCheck"] and (not humanoid or humanoid.Health <= 0)
                local has_forcefield = flags["ForceFieldCheck"] and character:FindFirstChildOfClass("ForceField") ~= nil
                local is_invincible = flags["InvincibleCheck"] and (humanoid and humanoid.MaxHealth >= 1e6)
                ...
                if is_teammate or is_invisible or is_dead or has_forcefield or is_invincible then
                    -- Hide main UI holder
