...
-- library functions 
	-- misc functions
		...
		do
			local _rawListfiles = listfiles
			listfiles = function(path)
				local results = _rawListfiles(path)
				local normalised = {}
				for i, file in ipairs(results) do
					normalised[i] = file:gsub("/", "\\")
				end
				return normalised
			end
		end
		...
		function library:config_list_update() 
			...
			local folder_path = (library.directory .. "/configs"):gsub("/", "\\")
		
			for _, file in next, listfiles(library.directory .. "/configs") do
				local name = file:gsub(folder_path .. "\\", ""):sub(1, -5)
				list[#list + 1] = name
			end
		...
		function library:window(properties)
			...
			-- main window
				local main_window = library:panel({
					name = properties and properties.name or "Origin | ", 
			...
			-- theming 
				...
				local watermark = library:watermark({default = os.date('Origin |  - %b %d %Y - %H:%M:%S')})  

				task.spawn(function()
					while task.wait(1) do 
						watermark.change_text(os.date('Origin - Beta - %b %d %Y - %H:%M:%S'))
					end 
				end) 
